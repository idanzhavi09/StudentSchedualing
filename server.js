...
class scheduleBlockClass {
    constructor(scheduleBlockID, DayOfWeek, startTime, RoomID, DegreeClassID , SemesterID , comments) {
...
connection.on("connect", err => {
    if (err) {
        ...
        app.post('/getScheduleBlock', (req, res) => {
            console.log('RECIEVED REQUEST TO RETRIEVE SCHEDULE BLOCK');
            let degree = req.body.DegreeName;
            getScheduleBlockArr(degree).then((result) => {
                res.send(result);
            })
            .catch((err) => {
                console.log('err:' + err);
    ...
    app.post('/submitCoursePlacement', (req , res) => {
        console.log('RECIEVED REQUEST TO SUBMIT COURSE PLACEMENT');
        let block = req.body.block;
        let degree = req.body.degree;
        submitCoursePlacement(block , degree).then((result) => {
            if(result === 200){
                res.sendStatus(200);
            }else{
                res.sendStatus(500);
        ...
        async function getClasses() {
            ...
            let promise = new Promise((resolve, reject) => {
                ...
                request.on('requestCompleted', () => {
                    console.log('REQUEST COMPLETEDD');
                    let finalResults = [];
                    if (results !== null) {
        ...
        async function getDegrees() {
            ...
            let promise = new Promise((resolve, reject) => {
                ...
                request.on('row', (columns) => {
                    columns.forEach((column) => {
                        if (column.value === null || column.value === undefined) {
                            console.log('VALUE WAS EITHER NULL OR UNDEFINED');
                        } else {
                            results.push(column.value);
                        }
                    })
        ...
        async function getScheduleBlockArr(Degree) {
            let results = [];

            let promise = new Promise((resolve, reject) => {
                let request = new Request('SELECT [ScheduleBlockID] ,[DayOfWeek] ,[StartTime] ,[RoomID] ,[DegreeClassID] ,[SemesterID] ,[Comments] FROM [dbo].[ScheduleBlock]', (err) => {
                        if (err) {
                            console.log('ERROR:' + err);
                        }
                    });
        ...
        async function submitCoursePlacement(block , degree){
            let promise = new Promise((resolve , reject) => {
                let request = new Request('' , (err) => {
                    if(err){
                        console.log(err);
                    }
                });
